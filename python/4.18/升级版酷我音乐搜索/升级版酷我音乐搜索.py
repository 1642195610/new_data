...
def ss():
    ...
    s = a1.get()
    ys = aa1.get()
    if a1.get() != "" and aa1.get() != "":
        t = 0
        os.mkdir(s)
        rn = str(30 * int(ys))
        for y in range(int(ys) - 1):
...
l2 = Label(a,text = "(每页30首歌曲!!!)请输入要下载的页数:",font = ("华文行楷",15)).grid(row = 1)
b = Tk()
Label(b, text="输入内容和页数按<确定>按钮,请稍等,直到弹窗出现,就下载完成啦!!!",width =90 ,height = 15,bg = "black",fg = "red",font = ("华文行楷",30)).pack()
